...
次のライブの曲を決めるとき「最近どの曲をやったか」「この曲は何回目か」が分からなくなる問題や、
**PA・照明の設定が記録に残らず、同じ曲でもどう鳴らした・どう照らしたか分からなくなる問題**を解決します。
...
| ライブ `/lives` | ライブをセトリ風カードで日付の新しい順に表示。カードを開くと曲順どおりのセットリスト、出演バンド名、登録者名、編集・削除。曲に PA・照明メモがあれば曲名の下に表示 |
| ライブ編集（モーダル） | ライブ名・出演バンド・日付・会場と曲。曲は「1曲ずつ（並べ替え・削除可）」と「まとめて貼り付け（1行1曲）」を切り替えられる。1曲ずつモードでは曲ごとに PA メモ・照明メモ（各任意）を入力できる |
...
- 同じライブで同じ曲を 2 回演奏した場合も記録でき、演奏回数は `count(distinct live_id)` で
  数えるので二重計上されません。
- **PA班・照明班向けに、曲ごとの設定メモ**（音響の EQ・エフェクト設定、照明の色・タイミングなど）
  を残せます。同じ曲でも会場や機材でやり方が変わるため、曲そのものではなく「このライブのこの曲」
  単位で記録し、次に同じ曲をやるときや別の担当者が確認するときの参考にできます。
...
- `npm run test` — Vitest（純粋関数の単体テスト 46 件）
